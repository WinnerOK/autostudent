...
def markdown_keypoints(video_url, keypoints) -> str:
    keypoints = json.loads(keypoints)
    timed_url_prefix = video_url.split('?')[0].replace('embed/', 'watch?v=') + "&t="
    md_content_str = ""
    for kp in keypoints:
        md_content_str += fmt.mlink(format_sec(kp['start_time']), timed_url_prefix + str(kp['start_time'])) + " " + fmt.mbold(fmt.escape_markdown(kp['content'] + '\n'))
