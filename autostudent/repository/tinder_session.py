...
async def start_session(
    ...
    session_id = await conn.fetchval(
        dedent(
            """
            insert into autostudent.user_tinder_session(user_id)
            values
            ($1)
    ...
    await conn.executemany(
        dedent(
            """
            insert into autostudent.tinder_session_queue(session_id, word, word_order)
            values ($1, $2, $3)
            """,
        ),
        insert_data,
...
async def stop_session(conn: asyncpg.Connection, session_id: int):
    await conn.execute(
        dedent(
            """
            delete
            from autostudent.user_tinder_session
            where session_id = $1
            """,
        ),
        session_id,
...
async def delete_word_from_session(
    ...
    await conn.execute(
        dedent(
            """
            delete
            from autostudent.tinder_session_queue
...
async def get_next_word_from_session(
    ...
    return await conn.fetchval(
        dedent(
            """
            delete from autostudent.tinder_session_queue
            where id in (
                select id
                    from autostudent.tinder_session_queue
                    where session_id = $1
